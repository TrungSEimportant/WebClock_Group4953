...
function Promo(name, value) { // khuyen mai
    ...
    this.toWeb = function() {
        ...
        switch (this.name) {
            case "giamgia":
                contentLabel = `<i class="fa fa-bolt"></i> Decrease ` + this.value + `&#8363;`;
                break;

            case "tragop":
                contentLabel = `Installment ` + this.value + `%`;
                break;

            case "giareonline":
                contentLabel = `Cheap prices online`;
                break;

            case "moiramat":
                contentLabel = "Newly released";
                break;
        }
...
function addToWeb(p, ele, returnString) {
    ...
    if (p.rateCount > 0) {
        ...
        rating += `<span>` + p.rateCount + ` Rating</span>`;
    }
