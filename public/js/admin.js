...
function layThongTinSanPhamTuTable(id) {
    ...
    if (isNaN(star)) {
        alert('Start must be an integer');
        return false;
    }

    if (isNaN(rateCount)) {
        alert('The rating number must be an integer.');
        return false;
    }
...
function themSanPham() {
    ...
    alert('Add product "' + newSp.name + '" successfully.');
...
function suaSanPham(masp) {
    ...
    alert('Edit ' + sp.name + ' successfully');
...
function addKhungSuaSanPham(masp) {
    ...
    s += `
        ...
        <tr>
            <td>Number of stars (integer 0->5):</td>
        ...
        <tr>
            ...
            <td>
                <select>
                    ...
                    <option value="giareonline" ` + (sp.promo.name == 'giareonline' ? 'selected' : '') + `>Cheap online</option>
                    <option value="moiramat" ` + (sp.promo.name == 'moiramat' ? 'selected' : '') + `>New Release</option>
                </select>
            </td>
        </tr>
        <tr>
        ...
        for (var j = 0; j < u[i].donhang.length; j++) {
            if (u[i].donhang[j].ngaymua == maDonHang) {
                if (duyetDon) {
                    ...
                    } else if (u[i].donhang[j].tinhTrang == 'Canceled') {
                        alert('Cancelled order cannot be approved!');
                        return;
                    }
                } else {
                    if (u[i].donhang[j].tinhTrang == 'Pending') {
                        if (window.confirm('Are you sure you want to cancel this order? This action cannot be reversed!'))
                            u[i].donhang[j].tinhTrang = 'Canceled';
...
function voHieuHoaNguoiDung(inp, taikhoan) {
    ...
    for (var u of listUser) {
        if (u.username == taikhoan) {
            ...
            setTimeout(() => alert(`${value ? 'Lock' : 'Unlock'} account ${u.username} successfully.`), 500);
            break;
        }
    }
...
function xoaNguoiDung(taikhoan) {
    if (window.confirm('Confirm deletion ' + taikhoan + '? \nAll data about ' + taikhoan + ' will be lost! Including orders of ' + taikhoan)) {
        var listuser = getListUser();
        for (var i = 0; i < listuser.length; i++) {
