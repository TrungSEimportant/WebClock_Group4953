...
function phanTich_URL_chiTietSanPham() {
    ...
    if (sanPhamHienTai.rateCount > 0) {
        ...
        rating += `<span> ` + sanPhamHienTai.rateCount + ` Rating</span>`;
    }
    ...
    var s = addThongSo('Brand', sanPhamHienTai.detail.screen);
    s += addThongSo('Object', sanPhamHienTai.detail.os);
    s += addThongSo('Water resistant', sanPhamHienTai.detail.camara);
    s += addThongSo('Glass material', sanPhamHienTai.detail.camaraFront);
    s += addThongSo('Size', sanPhamHienTai.detail.cpu);
    s += addThongSo('Machine type', sanPhamHienTai.detail.ram);
    s += addThongSo('Wire material', sanPhamHienTai.detail.rom);
    s += addThongSo('Thickness', sanPhamHienTai.detail.microUSB);
    s += addThongSo('Utilities', sanPhamHienTai.detail.battery);
    info.innerHTML = s;
...
function getDetailPromo(sp) {
    switch (sp.promo.name) {
        case 'tragop':
            ...
            return `Customers can buy products in installments with ` + span + `within 6 months from purchase.`;

        case 'giamgia':
            var span = `<span style="font-weight: bold">` + sp.promo.value + `</span>`;
            return `Customers will receive a discount ` + span + `₫ when buying directly at the store`;

        case 'moiramat':
            return `Customers will be able to try the device for free at the store. Defective items can be returned within 2 months.`;

        case 'giareonline':
            var del = stringToNum(sp.price) - stringToNum(sp.promo.value);
            var span = `<span style="font-weight: bold">` + numToString(del) + `</span>`;
            return `Products will be discounted ` + span + `₫ when purchasing online using VPBank card or SMS message`;

        default:
            var span = `<span style="font-weight: bold">61 xe Wave Alpha</span>`;
            return `Chance of winning ` + span + ` when paying Home Credit installments`;
    }
}
...
function suggestion() {
    ...
    if (sanPhamTuongTu.length) {
        ...
        addKhungSanPham(sanPhamTuongTu, 'You may like', ['#434aa8', '#ec1f1f'], div);
    }
}